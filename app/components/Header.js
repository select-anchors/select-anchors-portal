...
/** Reusable nav link with strong active styling */
function NavLink({ href, children }) {
  ...
  const active =
    "text-gray-900 font-extrabold border-b-2 border-[#2f4f4f]";
    ...
    <header className="site-header border-b border-gray-200 bg-white">
      <div className="container h-14 flex items-center justify-between px-4 md:px-6">
        ...
        <Link href="/" className="flex items-center gap-3">
          <div className="h-8 w-8 rounded-lg bg-[#2f4f4f] flex items-center justify-center text-white font-bold">
            SA
          </div>
      ...
      {/* Mobile menu (kept simple; no active style needed right now) */}
      {open && (
        <div className="md:hidden border-t border-gray-200">
          <div className="container py-3 flex flex-col gap-3 px-4 md:px-6">
            ...
            <Link href="/driver/my-day" onClick={() => setOpen(false)}>
              My Day
            </Link>
            <Link href="/wells/30-015-54321" onClick={() => setOpen(false)}>
              Well Detail
            </Link>
